...
int main(int argc, char** argv)
{
...

	{
		...
		auto fixDestAddress = [](std::string& destAdd) {
			destAdd = "john.smith@senemail.ca";
			};


		// TODO: Iterate over the array of messages, and apply the lambda expression to each message
			for (seneca::Message& msg : messages) {
				if(msg.m_toAddress=="")
					fixDestAddress(msg.m_toAddress);
			}
...

	{
		...
		for (auto i = 0u; i < server.getInboxSize() + 3; ++i)
		{
			...
			{
				auto& msg = server[i];
				std::cout << std::setw(2) << std::right<< i << ". " << msg.m_subject << "\n";
			}
